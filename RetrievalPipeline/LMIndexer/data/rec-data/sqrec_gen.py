...
def it2semantic_token(inputs, mode, max_len=None):    
    ...
    elif mode in ['rqvae', 'ours', 'ours-weak', 'rqvae-weak-1', 'rqvae-weak-2', 'ours-recon2', 'ours-recon3', 'ours-dim256', 'ours-dim128'] or 'rqvae' in mode:
        if isinstance(inputs, list):
    ...
    elif mode == 'tree' or "hcindexer" in mode:
        if isinstance(inputs, list):
...
if args.semantic_id_mode not in ['atomic', 'ours', 'ours-weak', 'ours-recon2', 'ours-recon3', 'ours-dim256', 'ours-dim128'] or 'rqvae' in args.semantic_id_mode or 'hcindexer' in args.semantic_id_mode:
    rowid2semanticid = json.load(open(os.path.join(args.data_dir, 'preprocess', f"{args.semantic_id_mode}-code-{base_name}.json")))
    if 'noconflict' not in args.semantic_id_mode:
